...
class Settings(BaseSettings):
    ...
    search_weight_lexical: float = 0.65
    ...
    search_embedding_retry_enabled: bool = True
    search_embedding_retry_max_attempts: int = 2
    search_embedding_retry_backoff_ms: int = 6
    search_embedding_retry_jitter_ratio: float = 0.25
    search_embedding_retry_budget_ms: int = 40
    search_embedding_quick_timeout_seconds: float = 0.35
    search_embedding_timeout_cooldown_enabled: bool = True
    search_embedding_timeout_cooldown_seconds: float = 20.0
    ...
    qdrant_longterm_age_days: int = 0
    qdrant_bootstrap_batch_size: int = 2000
    qdrant_bootstrap_ttl_seconds: int = 300
    qdrant_low_score_threshold: float = 0.42
    qdrant_low_hit_threshold: int = 2
    ...
    autonomy_gate_min_avg_context_quality: float = 0.70
    autonomy_gate_min_eval_floor: float = 0.58
    ...
    def redaction_zones(self) -> list[str]:
        return [path.strip() for path in self.redaction_zone_paths.split(",") if path.strip()]
